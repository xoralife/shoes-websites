    ...
    <section className="py-20 bg-white">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-[#16213E]">Frequently Asked Questions</h2>
          <p className="text-[#6C757D] mt-3">Got questions? We have answers.</p>
        </div>
        <div className="space-y-3">
          {faqs.map((faq, i) => (
            <div key={i} className="border border-gray-200 rounded-2xl overflow-hidden transition-all duration-300">
              ...
              >
                <span className="font-medium text-[#16213E]">{faq.q}</span>
                <ChevronDown size={18} className={`text-[#6C757D] transition-transform duration-300 ${open === i ? "rotate-180" : ""}`} />
              </button>
              <div className={`px-6 transition-all duration-300 ${open === i ? "pb-4 max-h-40" : "max-h-0 pb-0"} overflow-hidden`}>
                <p className="text-[#6C757D] text-sm leading-relaxed">{faq.a}</p>
              </div>
            </div>
          ))}
        </div>
