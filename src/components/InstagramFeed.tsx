    ...
    <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10">
          <Instagram size={28} className="mx-auto text-[#E94560] mb-3" />
          <h2 className="text-3xl md:text-4xl font-bold text-[#16213E]">Follow Us on Instagram</h2>
          <p className="text-[#6C757D] mt-2">@solemate_official</p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {posts.map((post) => (
            <div key={post.id} className="aspect-square rounded-2xl bg-gradient-to-br from-gray-100 to-gray-200 flex flex-col items-center justify-center card-hover cursor-pointer">
              ...
              <p className="text-xs text-[#6C757D]">{post.likes} likes</p>
            </div>
          ))}
        </div>
      </div>
