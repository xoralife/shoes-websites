      ...
      <main className="pt-24 pb-16">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3 mb-8">
            <Package size={28} className="text-[#E94560]" />
            <h1 className="text-2xl md:text-3xl font-bold text-[#16213E] dark:text-[#F8F9FA]">Order History</h1>
            <span className="text-[#6C757D] dark:text-gray-400 text-sm">({orders.length} orders)</span>
          </div>
          ...
          {orders.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-[#6C757D] dark:text-gray-400 mb-6">Start shopping to see your orders here.</p>
                ...
                <div key={order.id} className="bg-white dark:bg-[#1A1A2E] rounded-2xl card-shadow overflow-hidden">
                  <button onClick={() => setExpanded(expanded === order.id ? null : order.id)} className="w-full flex items-center justify-between p-5 text-left">
                    <div>
                      ...
                      <p className="text-xs text-[#6C757D] dark:text-gray-400 mt-0.5">{new Date(order.date).toLocaleDateString()} &middot; {order.items.length} items</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="font-bold text-[#16213E] dark:text-[#F8F9FA]">${order.total.toFixed(2)}</span>
                      <ChevronDown size={18} className={`text-[#6C757D] transition-transform duration-300 ${expanded === order.id ? "rotate-180" : ""}`} />
                    </div>
                  </button>
                  {expanded === order.id && (
                    <div className="px-5 pb-5 space-y-3 border-t border-gray-100 dark:border-[#2D2D4A] pt-4">
                      {order.items.map((item, i) => (
                        <div key={i} className="flex items-center justify-between text-sm">
                          <span className="text-[#16213E] dark:text-[#F8F9FA]">{item.name} <span className="text-[#6C757D]">x{item.quantity}</span></span>
                      ...
                      <div className="border-t border-gray-100 dark:border-[#2D2D4A] pt-3 space-y-1 text-sm">
                        <div className="flex justify-between text-[#6C757D]"><span>Shipping</span><span>{order.shipping === 0 ? "FREE" : `$${order.shipping.toFixed(2)}`}</span></div>
