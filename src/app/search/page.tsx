    ...
    <main className="pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form onSubmit={handleSearch} className="relative max-w-2xl mb-8">
          <Search size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#6C757D] dark:text-gray-400 dark:text-gray-500" />
          <input
            type="text"
            value={searchInput}
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-sm text-[#6C757D] dark:text-gray-400 dark:text-gray-500">{results.length} results for &ldquo;{query}&rdquo;</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 flex-wrap">
              {categories.map((cat) => (
                <button key={cat} onClick={() => setCatFilter(cat)} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${catFilter === cat ? "bg-[#E94560] text-white" : "bg-gray-100 dark:bg-[#2D2D4A] text-[#6C757D] dark:text-gray-400 dark:text-gray-500"}`}>{cat}</button>
              ))}
            </div>
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="px-4 py-2 rounded-xl border border-gray-200 dark:border-[#2D2D4A] bg-white dark:bg-[#1A1A2E] text-sm text-[#6C757D] dark:text-gray-400 dark:text-gray-500 focus:outline-none">
        ...
        {results.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-[#6C757D] dark:text-gray-400 dark:text-gray-500">Try a different search term.</p>
          </div>
        ) : (
...
export default function SearchPage() {
  return (
    <div className="min-h-screen bg-[#F8F9FA] dark:bg-[#0F0F1A]">
      <Header />
      <Suspense fallback={<div className="pt-32 text-center text-[#6C757D] dark:text-gray-400 dark:text-gray-500">Loading...</div>}>
        <SearchContent />
      </Suspense>
      <MobileBottomNav />
      <CartSidebar />
